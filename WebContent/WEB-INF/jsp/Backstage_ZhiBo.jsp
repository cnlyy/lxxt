			...
			<div class="all_link_box">
				<table class="all_link_title">
					...
					<c:forEach items="${pagemsg.lists}" var="u">
						<tr class="tr_link">
							<td class="link1"><p class="p_link1">${u.id }</p></td>
							<td class="link2" style=""><p class="p_link2">${u.link }</p></td>
							<td class="link3"><p class="p_link3">${u.uploader }</p></td>
							<td class="link3"><a href="deleteLive.action?id=${u.id }"><button>&nbsp;删&nbsp;除&nbsp;</button></a></td>
						</tr>
					</c:forEach>
				</table>
					...
					<tr>
						...
						<td class="td2"><span>第${pagemsg.currPage }/
								...
								每页显示:${pagemsg.pageSize}</span> <span> <c:if
									...
									<a
										href="${pageContext.request.contextPath }/Backstage_ZhiBo.action?currentPage=1">[首页]</a>
									<a
										href="${pageContext.request.contextPath }/Backstage_ZhiBo.action?currentPage=${pagemsg.currPage-1}">[上一页]</a>
								</c:if> <c:if
									test="${requestScope.pagemsg.currPage != requestScope.pagemsg.totalPage}">
									<a
										href="${pageContext.request.contextPath }/Backstage_ZhiBo.action?currentPage=${pagemsg.currPage+1}">[下一页]</a>
									<a
										href="${pageContext.request.contextPath }/Backstage_ZhiBo.action?currentPage=${pagemsg.totalPage}">[尾页]</a>
								</c:if>
						</span></td>
					</tr>
				</table>
